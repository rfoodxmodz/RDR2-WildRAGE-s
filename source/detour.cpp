...
void PatchInJump(uint64_t address, void* destination) {
	...
	*(uint64_t*)(address + 6) = (uint64_t)destination;
	...
	//memcpy_p((address), "\xFF\x25\x00\x00\x00\x00", 6);
	//sys_proc_rw((address + 6), destination, sizeof(destination));
}
char stub_buf[255];
void* DetourFunction(uint64_t address, void* destination, int length) {
	...
	//sprintf(stub_buf, "stubAddress: %llx", stubAddress);
	
	//sceSysUtilSendSystemNotificationWithText(222, stub_buf);
